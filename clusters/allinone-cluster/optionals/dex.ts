	...
	.letIf(([_, ns, rawEtcd, rawLdap, rawIstio]) => {
		...
		const staticClients = {
			...
			"prompt-forge": {
				id: "prompt-forge",
				redirectURIs: [
					"https://prompt-forge.egoavara.net/auth/callback",
					"http://localhost:5173/auth/callback",
				],
				name: "Prompt Forge",
				secret: new RandomPassword("prompt-forge-static-client", {
					length: 24,
					special: false,
					overrideSpecial: "_",
				}).result,
			...
			"mcp-orchestrator": {
				id: "mcp-orchestrator",
				redirectURIs: [
					"https://mcp-orchestrator.egoavara.net/oauth2/callback",
					"https://mcp-orchestrator.egoavara.net/",
					"http://127.0.0.1:3000/oauth/callback",
					"http://127.0.0.1:3000/oauth/callback",
				],
				name: "MCP Orchestrator",
				secret: new RandomPassword("mcp-orchestrator-static-client", {
					length: 24,
					special: false,
					overrideSpecial: "_",
				}).result,
		...
		const svc = core.v1.Service.get(
			"dex",
			interpolate`${chart.namespace}/${chart.name}`,
			{ dependsOn: [chart] },
		);
		useWaypoint(svc);
